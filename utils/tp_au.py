...
class PolypObjDataset(data.Dataset):
    def __init__(self, image_root, gt_root, trainsize, type_='train'):
        ...
        if type_ == 'train':
            self.trans = albu.Compose([
                ...
                RandomCopyMove(p=0.1,always_apply=False),
                RandomInpainting(p=0.1,always_apply=False),
    ...
    def __getitem__(self, index):
        ...
        augmented = self.trans(image=np.array(image), mask=np.array(gt))
...
def get_loader(image_root, gt_root, batchsize, trainsize, type_='train',
               shuffle=True, num_workers=12, pin_memory=True):
    dataset = PolypObjDataset(image_root, gt_root, trainsize, type_)
...
class test_dataset:
    def __init__(self, image_root, gt_root, testsize):
        ...
        self.gt_transform = transforms.Compose([
            transforms.Resize((self.testsize, self.testsize)),
            transforms.ToTensor()])
        self.gt2_transform = transforms.Compose([
        transforms.ToTensor()])
        self.size = len(self.images)
        self.index = 0
    ...
    def load_data(self):
        ...
        return image, gt, name, basic_size #np.array(image_for_post)
