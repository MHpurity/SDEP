...
class PolypObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        augmented = self.trans(image=np.array(image), masks=[np.array(gt)])
        ...
        image_np = image.resize((self.trainsize, self.trainsize), Image.BILINEAR)
        image_np = np.array(image_np)

        gt_np = gt.resize((self.trainsize, self.trainsize), Image.NEAREST)
        gt_np = np.array(gt_np)

        return image_tensor, image_np, gt_tensor, gt_np, image_name #同时返回Numpy格式的数据
        # return image_tensor, gt_tensor, eg_tensor
...
class test_dataset:
    def __init__(self, image_root, gt_root, testsize):
        ...
        self.images = [image_root + f for f in os.listdir(image_root) if f.endswith('.jpg') or f.endswith('.png') or f.lower().endswith('.tif') or f.endswith('.bmp') or f.endswith('.jpeg')]
        self.gts = [gt_root + f for f in os.listdir(gt_root) if f.lower().endswith('.tif') or f.lower().endswith('.png') or f.endswith('.tif') or f.endswith('.bmp') or f.endswith('.jpg')]
        self.images = sorted(self.images)
        self.gts = sorted(self.gts)
    ...
    def load_data(self):
        ...
        image_numpy = image.resize((self.testsize, self.testsize), Image.BILINEAR)
        image_numpy = np.array(image_numpy)
        # print(f'index={self.index}')
        gt = self.binary_loader(self.gts[self.index])
        gt_numpy = gt.resize((self.testsize, self.testsize), Image.BILINEAR)
        ...
        return image_tensor, image_numpy, gt, name, np.array(image_for_post)
